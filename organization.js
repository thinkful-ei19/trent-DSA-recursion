...
const organization = (org, boss) => {
  let node = {};
  org.filter(employee => employee.boss === boss)
    .forEach(emp => node[emp.id] = organization(org, emp.id));
  return node;
};


const structuredOrg = organization(org, null);

const traverseR = (obj, depth) => {
  let indent = ' '.repeat(depth * 4);
  Object.keys(obj).forEach(item => {
    console.log(indent + item);
    return traverseR(obj[item], depth + 1);
  });
};


console.log(traverseR(structuredOrg, 0));